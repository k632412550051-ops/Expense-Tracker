...
export function PieChartWidget({ expenses, title = "Cơ cấu chi tiêu tháng này", categoryColors }: PieChartWidgetProps) {
  ...
  if (data.length === 0) {
    return (
      <div className="liquid-glass rounded-3xl p-6 relative shadow-xl shadow-blue-950/5 border border-white/85 flex flex-col h-[350px] sm:h-[400px] overflow-hidden">
        ...
        <h2 className="text-base sm:text-lg font-extrabold font-heading text-slate-900 mb-6 truncate tracking-tight" title={title}>{title}</h2>
        <div className="flex-1 flex items-center justify-center text-xs sm:text-sm text-slate-400 font-medium text-center">
          Chưa có dữ liệu
        </div>
      </div>
    );
  ...
  return (
    <div className="liquid-glass rounded-3xl p-6 relative shadow-xl shadow-blue-950/5 border border-white/85 flex flex-col min-h-[400px] overflow-hidden">
      <div className="absolute inset-x-0 top-0 h-[1.5px] bg-gradient-to-r from-transparent via-white to-transparent opacity-90 pointer-events-none" />
      <h2 className="text-base sm:text-lg font-extrabold font-heading text-slate-900 mb-2 truncate tracking-tight" title={title}>{title}</h2>
      ...
      {/* Custom Legend */}
      <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-2.5 mt-4 pt-4 border-t border-blue-100/40">
        {data.map((entry, index) => (
          <div key={`legend-${index}`} className="flex items-center gap-2">
            ...
            />
            <span className="text-slate-700 font-bold text-xs sm:text-sm truncate max-w-[150px]">{entry.name}</span>
          </div>
        ))}
      </div>
    </div>
